...
func (s *SystemService) getConfigValue(key string, defaultValue interface{}) interface{} {
	if !s.config.IsSet(key) {
		s.Logger.LogWarn("配置项不存在，使用默认值", zap.Skip())
		return defaultValue
	}
...
func (s *SystemService) UpdateSystemConfig(config system.SystemConfig) error {
	s.Logger.LogInfo("开始更新系统配置", zap.String("operation", "config_update"))
	...
	if err := s.validateConfig(config); err != nil {
		s.Logger.LogError("配置验证失败", zap.Skip(), zap.Error(err))
		return fmt.Errorf("配置验证失败: %w", err)
	}
	if err := s.config.WriteConfig(); err != nil {
		s.Logger.LogError("写入配置文件失败", zap.Skip(), zap.Error(err))
		return fmt.Errorf("写入配置文件失败: %w", err)
	}
	s.Logger.LogInfo("系统配置更新成功", zap.String("operation", "config_update"))
	return nil
}
...
func (s *SystemService) GetSystemHealth() map[string]interface{} {
	...
	if err != nil {
		...
		s.Logger.LogError("获取系统指标失败", zap.Skip(), zap.Error(err))
	} else {
		if mem, ok := metrics["memory"].(map[string]interface{}); ok {
			if usedPercent, ok := mem["used_percent"].(float64); ok && usedPercent > 90 {
				...
				s.Logger.LogWarn("内存使用率过高", zap.Skip(), zap.Float64("used_percent", usedPercent))
			}
		}
		if cpu, ok := metrics["cpu"].(map[string]interface{}); ok {
			if usagePercent, ok := cpu["usage_percent"].(float64); ok && usagePercent > 80 {
				...
				s.Logger.LogWarn("CPU负载过高", zap.Skip(), zap.Float64("usage_percent", usagePercent))
			}
		}
	}
...
func (s *SystemService) GetSystemMetrics() (map[string]interface{}, error) {
	...
	if err != nil {
		s.Logger.LogError("获取内存信息失败", zap.Skip(), zap.Error(err))
		return nil, fmt.Errorf("获取内存信息失败: %w", err)
	}
	s.BaseService.Logger.LogInfo("系统服务初始化完成", zap.String("service", "system"))
	...
	for i := 0; i < 3; i++ {
		...
		s.Logger.LogWarn("获取CPU信息失败,正在重试", zap.Skip(), zap.Error(err))
		time.Sleep(time.Second)
	}
	if err != nil {
		s.Logger.LogError("获取CPU信息失败", zap.Skip(), zap.Error(err))
...
func (s *SystemService) checkRedisHealth() string {
	...
	if result.Err() != nil {
		s.Logger.LogWarn("Redis连接异常", zap.Error(result.Err()))
		return "down"
	}
	return "up"
}
